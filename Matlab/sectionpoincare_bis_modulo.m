function sectionpoincare_bis_modulo()
...
for i= 1:tmax
   ...
   subplot(2,1,1);
   box on;hold on;
   title('Section de Poincare stroboscopique  - echantillonnage: ');
   ...
   subplot(2,1,2);
   box on;hold on;
   title('Section de Poincare stroboscopique  - Modulo 2*\pi');
   xlabel('\theta'); ylabel('d\theta/dt');
...
f = getframe(gcf);
imwrite(f.cdata,['SP_modulo=',num2str(k),num2str(k1),num2str(k2),num2str(A),num2str(omega),'.png']);